...
def merge_similar_sentences(data):
    ...
    for i in range(1, len(data)):
        ...
        if (
            difflib.SequenceMatcher(None, current_sentence, next_sentence).ratio() > 0.98
            and -1 <= next_timestamp[0] - current_timestamp[1] <= 1
        ):
...
def parse_natural_language_answer(answer):
    segments = re.findall(r'(.*?)(?:from|From) (\d{2}) to (\d{2})\.', answer)
...
def captioning_metrics(all_logs, data_path):
    ...
    for log in logs:
        ...
        pred[video_id] = []
        try:
            if '[' in answer and 'event' in answer:
                items = json.loads(re.search(r'\[.*\]', answer).group(0))
                for item in items:
                    pred[video_id].append({
                        'timestamp': [int(item['timestamps'][5:7]), int(item['timestamps'][-2:])],
                        'sentence': item['event'],
                    })
            else:
                pred[video_id] = parse_natural_language_answer(answer)
    ...
    for vid, items in list(pred.items()):
        items = merge_similar_sentences(items)
        duration = gt_js[vid]['duration']
        for item in items:
...
def print_parsed_predictions(pred):
    print("\n========= Parsed Predictions =========")
    for vid, items in pred.items():
        print(f"\nVideo ID: {vid}")
        for item in items:
            ts = item['timestamp']
            print(f" - [{ts[0]:.2f}s ~ {ts[1]:.2f}s] {item['sentence']}")
...
if __name__ == "__main__":
    ...
    if args.task in ['captioning', 'all']:
        print("====================== Captioning =====================")
        metrics, parsed_preds = captioning_metrics(logs, args.data_path)
        print_parsed_predictions(parsed_preds)
