...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            # feature_path = '{}/{}.pkl'.format(self.data_args.feat_folder, source['video_name'])
            # feature_path = '{}/{}.npy'.format(self.data_args.feat_folder, source['id'])
            ##########################################################################################
            if len(source['feat_path']) != 1:
                feature_path_1 = '{}/{}'.format(self.data_args.feat_folder, source['feat_path'][0])
                feature_path_2 = '{}/{}'.format(self.data_args.feat_folder, source['feat_path'][1])

                image_1 = np.load(feature_path_1, allow_pickle=True) # <N, 768> float16
                image_1 = torch.from_numpy(image_1)
                image_1 = image_1.squeeze(1)

                image_2 = np.load(feature_path_2, allow_pickle=True) # <N, 768> float16
                image_2 = torch.from_numpy(image_2)
                image_2 = image_2.squeeze(1)

                image_1 = image_1[int(source['start_time']):, :]
                image_2 = image_2[:int(source['stop_time']), :]
                
                image = torch.cat([image_1, image_2], dim=0)

                start_time = 0
                stop_time = image_1.shape[0] - int(source['start_time']) + int(source['stop_time'])

            else:
                feature_path = '{}/{}'.format(self.data_args.feat_folder, source['feat_path'][0])

                image = np.load(feature_path, allow_pickle=True) # <N, 768> float16
                image = torch.from_numpy(image)
                image = image.squeeze(1)

                start_time = int(source['start_time'])
                stop_time = int(source['stop_time'])

            if stop_time - start_time > 30:
                uniform_sampled_frames = np.linspace(start_time, stop_time - 1, num=30, dtype=int)
                image = image[uniform_sampled_frames, :]
            else:
                image = image[start_time : stop_time, :]
            ##########################################################################################
